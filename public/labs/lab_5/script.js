...
//     event.preventDefault();
//     // console.log('submit fired', search.value);
//     const filtered= data.filter((record) => record.zip.includes(searchInput.value) && record.geocoded_column_1);
...
//       // const marker= L.marker(result.geocoded_column_1.coordinates.reverse()).addTo(mapFromLeaflet);

//       const appendItem= document.createElement('li');
//       appendItem.classList.add('block');
//       appendItem.classList.add('list-item');
//       appendItem.innerHTML= `<div class="list-he ader is-size-5">${item.name}</div><address class="is-size-6">${item.address_line_1}</address>`;
